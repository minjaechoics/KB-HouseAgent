"""Download MOLIT single/multi-family trade transaction data to CSV.
...
Example:
    ...
    py -3 scripts/download_rtms_sh_trade.py --months 202401 --num-rows 100
"""
...
DEFAULT_OUT = OUT_DIR / "rtms_sh_trade.csv"
DEFAULT_META = OUT_DIR / "rtms_sh_trade_download_meta.json"
...
def main() -> int:
    ...
    print(f"[rtms-sh-trade] rows={len(df)} out={args.out}")
    print(f"[rtms-sh-trade] meta={args.meta}")
    if events and any(e.get("http_status") != 200 for e in events):
        print("[rtms-sh-trade] stopped because the API returned a non-200 response.")
        return 2
    return 0

"""Download MOLIT officetel rent transaction data to CSV.
...
Example:
    ...
    py -3 scripts/download_rtms_offi_rent.py --start 202407
"""
...
DEFAULT_OUT = OUT_DIR / "rtms_offi_rent.csv"
DEFAULT_META = OUT_DIR / "rtms_offi_rent_download_meta.json"
...
EXPECTED_COLUMNS = [
    ...
    "dealDay", "dealMonth", "dealYear", "deposit", "excluUseAr", "floor",
    "jibun", "monthlyRent", "offiNm", "preDeposit", "preMonthlyRent",
    "sggCd", "sggNm", "umdNm", "useRRRight",
]
...
def main() -> int:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for fut in as_completed(future_map):
                ...
                if done % 50 == 0 or done == len(tasks):
                    print(f"[rtms-offi-rent] {done}/{len(tasks)} tasks complete")
    ...
    print(f"[rtms-offi-rent] rows={len(df)} out={args.out}")
    print(f"[rtms-offi-rent] meta={args.meta}")
    print(f"[rtms-offi-rent] failures={len(failures)}")

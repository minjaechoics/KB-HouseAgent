"""Download MOLIT officetel trade transaction data to CSV.
...
Example:
    ...
    py -3 scripts/download_rtms_offi_trade.py --start 202407
"""
...
DEFAULT_OUT = OUT_DIR / "rtms_offi_trade.csv"
DEFAULT_META = OUT_DIR / "rtms_offi_trade_download_meta.json"
...
EXPECTED_COLUMNS = [
    ...
    "estateAgentSggNm", "excluUseAr", "floor", "jibun", "offiNm",
...
def main() -> int:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for fut in as_completed(future_map):
                ...
                if done % 50 == 0 or done == len(tasks):
                    print(f"[rtms-offi-trade] {done}/{len(tasks)} tasks complete")
    ...
    print(f"[rtms-offi-trade] rows={len(df)} out={args.out}")
    print(f"[rtms-offi-trade] meta={args.meta}")
    print(f"[rtms-offi-trade] failures={len(failures)}")

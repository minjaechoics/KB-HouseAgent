"""Download MOLIT apartment trade transaction data to CSV.
...
Example:
    ...
    py -3 scripts/download_rtms_apt_trade.py --start 202407
"""
...
DEFAULT_OUT = OUT_DIR / "rtms_apt_trade.csv"
DEFAULT_META = OUT_DIR / "rtms_apt_trade_download_meta.json"
...
EXPECTED_COLUMNS = [
    "lawd_cd", "deal_ym", "aptDong", "aptNm", "buildYear", "buyerGbn",
    "cdealDay", "cdealType", "dealAmount", "dealDay", "dealMonth",
    "dealYear", "dealingGbn", "estateAgentSggNm", "excluUseAr", "floor",
...
def main() -> int:
        ...
        with ThreadPoolExecutor(max_workers=args.workers) as executor:
            ...
            for fut in as_completed(future_map):
                ...
                if done % 50 == 0 or done == len(tasks):
                    print(f"[rtms-apt-trade] {done}/{len(tasks)} tasks complete")
    ...
    print(f"[rtms-apt-trade] rows={len(df)} out={args.out}")
    print(f"[rtms-apt-trade] meta={args.meta}")
    print(f"[rtms-apt-trade] failures={len(failures)}")
